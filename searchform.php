...
<form role="search" <?php echo $dfdl_aria_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/search/' ) ); ?>">
	...
	<input type="image" src="<?php echo get_stylesheet_directory_uri() ?>/assets/media/icon-search.svg" class="search-submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'dfdl' ); ?>" method="get" action="<?php echo get_home_url('', '/search/') ?>" />
</form>
